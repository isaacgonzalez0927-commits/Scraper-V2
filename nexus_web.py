...
@nexus_bp.route("/manifest.webmanifest")
def manifest():
    return jsonify({
        "name": "Nexus",
        "short_name": "Nexus",
...
@nexus_bp.route("/manifest-owner.webmanifest")
def manifest_owner():
    return jsonify({
        "name": "Nexus Dashboard",
        "short_name": "Nexus",
        "description": "Nexus owner dashboard — call tracking and reports.",
        "start_url": "/nexus/dashboard",
        "scope": "/nexus/",
        "display": "standalone",
        "orientation": "portrait",
        "background_color": "#f8f9fb",
...
@nexus_bp.route("/apple-touch-icon.png")
@nexus_bp.route("/apple-touch-icon-precomposed.png")
...
@nexus_bp.route("/generate", methods=["POST"])
def generate():
    if ACCESS_CODE:
...
@nexus_bp.route("/reset-history", methods=["POST"])
def reset_history():
    save_history(set())
    return jsonify({"ok": True})


@nexus_bp.route("/api/log-call", methods=["POST"])
def api_log_call():
    if not check_caller():
        return jsonify({"error": "unauthorized"}), 401
    data = request.get_json(silent=True) or {}
...
@nexus_bp.route("/dashboard")
def dashboard_page():
    return DASHBOARD_PAGE


@nexus_bp.route("/reports")
def reports_page():
    return REPORTS_PAGE


@nexus_bp.route("/history")
def history_page():
    return HISTORY_PAGE


@nexus_bp.route("/stats")
def stats_page():
    return STATS_PAGE


@nexus_bp.route("/api/dashboard")
def api_dashboard():
    if not check_owner():
        return jsonify({"error": "unauthorized"}), 401
...
@nexus_bp.route("/api/restore", methods=["POST"])
def api_restore():
    if not check_owner():
        return jsonify({"error": "unauthorized"}), 401
...
<title>Nexus</title>
<link rel="manifest" href="/nexus/manifest.webmanifest">
""" + NEXUS_ICON_LINKS + """
<style>
...
genBtn.addEventListener("click", async () => {
  ...
  try {
    const res = await fetch("/nexus/generate", {
      method:"POST",
...
async function poll(jobId, tries=0){
  try {
    const res = await fetch("/nexus/status/" + jobId);
...
async function logOutcome(idx, outcome, btn){
  ...
  btn.disabled = true;
  try {
    await fetch("/nexus/api/log-call", {
      method:"POST",
      headers:{"Content-Type":"application/json", "X-Access-Code": code},
      body: JSON.stringify({
...
document.getElementById("resetBtn").addEventListener("click", async () => {
  await fetch("/nexus/reset-history",{method:"POST"});
...
# Nexus is mounted by sere.create_app at /nexus
