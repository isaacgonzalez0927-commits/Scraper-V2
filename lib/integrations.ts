...
import { openaiFromEnv } from "./openai";
...
/**
 * Serenity and the star assistant always use the operator key. Shop-pasted
 * OpenAI rows are ignored so a customer cannot bring their own key, or
 * accidentally pay OpenAI themselves.
 */
export async function openaiConfig(_organizationId?: number): Promise<OpenAIConfig | null> {
