...
test("Connect Stripe and Connect Square open Integrations, not an inline paste box", async () => {
  const { CONNECT_STRIPE_HREF, CONNECT_SQUARE_HREF } = await import("../components/ConnectStripe");
  assert.equal(CONNECT_STRIPE_HREF, "/settings?tab=integrations#stripe");
  assert.equal(CONNECT_SQUARE_HREF, "/settings?tab=integrations#square");

  const { safeHashId } = await import("../lib/hash-scroll");
  assert.equal(safeHashId("#stripe"), "stripe");
  assert.equal(safeHashId("#square"), "square");
  assert.equal(safeHashId("#quickbooks"), "quickbooks");
  assert.equal(safeHashId("#<script>"), null);
});
...
test("Stripe connect is Create the Sere key, then paste", async () => {
  ...
  assert.match(tutorial, /btn-connect-hero/);
  assert.match(tutorial, /Never paste/);
  assert.match(tutorial, /sk_/);
  assert.equal(tutorial.includes("key-steps"), false);
  assert.equal(tutorial.includes("key-guide-lede"), false);
  ...
  assert.match(square, /production access token/);
  assert.equal(square.includes("key-steps"), false);
});
