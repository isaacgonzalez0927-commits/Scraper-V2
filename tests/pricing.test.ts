...
test("Shop is the office book; Crew is what $79 should buy", () => {
  ...
  assert.ok(shop?.features.some((f) => /house file crm/i.test(f.text)));
  assert.ok(shop?.features.some((f) => /collect:/i.test(f.text)));
  ...
  assert.ok(shop?.features.some((f) => /jobber or housecall/i.test(f.text)));
