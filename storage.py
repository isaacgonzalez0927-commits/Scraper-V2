"""Nexus auto-save — local snapshots + optional GitHub/URL remote sync.

Survives Render free-tier restarts when remote backup is configured.
"""
...
# Or generic: NEXUS_BACKUP_URL + optional NEXUS_BACKUP_TOKEN (GET/PUT JSON)
...
def build_snapshot() -> dict:
    ...
    data = tracking.export_backup()
    data["version"] = SNAPSHOT_VERSION
    data["saved_at"] = _now_iso()
    data["app"] = "nexus"
    if HISTORY_FILE.exists():
        try:
            data["generated_history"] = json.loads(
                HISTORY_FILE.read_text(encoding="utf-8")
            )
        except (json.JSONDecodeError, OSError):
            data["generated_history"] = {"phone_keys": []}
    else:
        data["generated_history"] = {"phone_keys": []}
    if LEARN_CACHE_FILE.exists():
        try:
            data["learn_cache"] = json.loads(
                LEARN_CACHE_FILE.read_text(encoding="utf-8")
            )
        except (json.JSONDecodeError, OSError):
            pass
    return data
...
def _github_config() -> tuple[str, str, str] | None:
    token = os.getenv("NEXUS_GITHUB_TOKEN", "").strip()
    repo = os.getenv("NEXUS_GITHUB_REPO", "").strip()
    if not token or not repo:
        return None
    path = os.getenv("NEXUS_GITHUB_PATH", "nexus-snapshot.json").strip()
...
def _fetch_remote_once() -> dict | None:
    ...
    url = os.getenv("NEXUS_BACKUP_URL", "").strip()
    if not url:
        return None
    try:
        import requests
        ...
        headers = {}
        tok = os.getenv("NEXUS_BACKUP_TOKEN", "").strip()
        if tok:
...
def push_remote_snapshot(snap: dict) -> bool:
    ...
    if gh:
        ...
        try:
            ...
            payload = {
                "message": f"Nexus auto-save {_now_iso()[:19]}",
...
def _snapshot_mtime() -> str | None:
    if not SNAPSHOT_FILE.exists():
        return None
    ts = SNAPSHOT_FILE.stat().st_mtime
