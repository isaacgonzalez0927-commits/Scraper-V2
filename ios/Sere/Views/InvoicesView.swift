import SwiftUI

struct InvoicesView: View {
    @State private var rows: [InvoiceRow] = []
    @State private var query = ""
    @State private var error = ""
    @State private var loaded = false

    private var filtered: [InvoiceRow] {
        let needle = query.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        guard !needle.isEmpty else { return rows }
        return rows.filter {
            $0.number.lowercased().contains(needle)
                || $0.customer.lowercased().contains(needle)
                || $0.statusLabel.lowercased().contains(needle)
        }
    }

    var body: some View {
        NavigationStack {
            List {
                ForEach(filtered) { row in
                    NavigationLink {
                        ShopWebView(path: row.href, title: row.number)
                    } label: {
                        VStack(alignment: .leading, spacing: 4) {
                            HStack {
                                Text(row.number).font(.headline)
                                Spacer()
                                Text(row.balance).font(.subheadline.monospacedDigit())
                            }
                            Text("\(row.customer) · due \(row.due)")
                                .font(.subheadline)
                                .foregroundStyle(.secondary)
                            StatusChip(text: row.statusLabel, tone: statusTone(row.status))
                        }
                        .padding(.vertical, 4)
                    }
                }
            }
            .listStyle(.insetGrouped)
            .navigationTitle("Invoices")
            .searchable(text: $query, prompt: "Search invoices")
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    NavigationLink {
                        ShopWebView(path: "/invoices/new", title: "New invoice")
                    } label: {
                        Image(systemName: "plus")
                    }
                }
            }
            .overlay {
                if !loaded && error.isEmpty {
                    ProgressView()
                } else if rows.isEmpty && error.isEmpty {
                    ContentUnavailableView(
                        "No invoices yet",
                        systemImage: "doc.text",
                        description: Text("Write the first one with the plus.")
                    )
                }
            }
            .safeAreaInset(edge: .bottom) {
                if !error.isEmpty { Text(error).font(.footnote).foregroundStyle(.red).padding() }
            }
            .task { await load() }
            .refreshable { await load() }
        }
    }

    private func load() async {
        do {
            rows = try await APIClient.shared.invoices()
            error = ""
        } catch {
            self.error = error.localizedDescription
        }
        loaded = true
    }
}
